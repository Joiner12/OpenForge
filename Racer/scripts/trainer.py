...
class NeuralNetwork:
    def __init__(self, input_size, hidden_size1, hidden_size2, output_size):
        ...
        self.input_weights = np.random.randn(input_size, hidden_size1).astype(
            np.float32
        ) * np.sqrt(2.0 / input_size)
        self.hidden_weights1 = np.random.randn(hidden_size1, hidden_size2).astype(
            np.float32
        ) * np.sqrt(2.0 / hidden_size1)
        self.hidden_weights2 = np.random.randn(hidden_size2, output_size).astype(
            np.float32
        ) * np.sqrt(2.0 / hidden_size2)
    ...
    def forward(self, X):
        ...
        self.hidden2 = self.relu(
            np.dot(self.hidden1, self.hidden_weights1) + self.hidden_bias2
        )
        self.output = self.softmax(
            np.dot(self.hidden2, self.hidden_weights2) + self.output_bias
        )
        return self.output
    ...
    def backward(self, X, y, output, learning_rate):
        ...
        self.hidden_bias2 += learning_rate * np.sum(
            hidden2_delta, axis=0, keepdims=True
        )
        self.hidden_bias1 += learning_rate * np.sum(
...
def parse_input(file_path):
    ...
    with open(file_path, "r") as file:
        for line in file:
            match = re.search(
                r"Red: (\d+), Green: (\d+), Blue: (\d+), Clear: (\d+), Color: (\w+)",
                line,
            )
            if match:
...
print(
    "\nuint32_t hidden_bias1[HIDDEN_SIZE1] = {"
    + ", ".join(float_to_hex(x) for x in nn.hidden_bias1.flatten())
    + "};"
)
print(
    "\nuint32_t hidden_bias2[HIDDEN_SIZE2] = {"
    + ", ".join(float_to_hex(x) for x in nn.hidden_bias2.flatten())
    + "};"
)
print(
    "\nuint32_t output_bias[OUTPUT_SIZE] = {"
    + ", ".join(float_to_hex(x) for x in nn.output_bias.flatten())
    + "};"
)
